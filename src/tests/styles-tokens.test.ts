 ...
 *
 * Three properties are pinned here, and they are what the rest of Track C builds on:
 *
 ...
 *
 *  2. **`--text-3` still clears WCAG AA on every surface.** §4.1's note makes
 *     this token contrast-pinned rather than chosen by taste, and its margin is
 *     thin (+0.023 at the worst pairing), so a nudge to `--text-3` *or to any
 *     of the four surfaces* can breach it. `npm run visual:checks` enforces the
 *     same bound, but that needs a browser boot; the fast suite must catch it.
 *
 *  3. **§4.6 purity holds for every non-token stylesheet.** `tokens.css` is the
 *     only definition site; every other file under `src/styles/` may reference
 *     tokens but may not carry colour, timing, radius, type or spacing literals.
 */
...
describe('§4.1 note — --text-3 clears WCAG AA on every surface', () => {
  // WCAG 2.1 relative luminance and contrast ratio. AA for normal text is
  // 4.5:1, the same bound CONFIG.CONTRAST_TEXT applies in visual:checks.
  ...
  const cases = (['dark', 'light'] as const).flatMap((theme) =>
    SURFACES.map((surface) => ({ theme, surface, vars: theme === 'dark' ? dark : light })),
  );

  it.each(cases)('$theme --text-3 on $surface', ({ surface, vars }) => {
    expect(ratio(vars['--text-3'], vars[surface])).toBeGreaterThanOrEqual(AA_NORMAL_TEXT);
  });

  it('checks all eight pairings and none vacuously', () => {
    expect(cases).toHaveLength(8);
    for (const { surface, vars } of cases) {
      expect(vars['--text-3']).toMatch(/^#[0-9a-f]{6}$/);
