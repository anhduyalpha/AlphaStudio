...
describe('validateClipboardImageBlob', () => {
  it('rejects empty', () => {
    ...
    const r = validateClipboardImageBlob(empty);
  ...
  it('rejects oversized', () => {
    ...
    const r = validateClipboardImageBlob(big);
    assert.equal(r.ok, false);
  ...
  it('rejects non-image', () => {
    ...
    const r = validateClipboardImageBlob(text);
    assert.equal(r.ok, false);
    assert.equal(r.code, 'NOT_IMAGE');

    const app = { size: 100, type: 'application/octet-stream' };
    const a = validateClipboardImageBlob(app);
    assert.equal(a.ok, false);
    assert.equal(a.code, 'NOT_IMAGE');
  });

  it('accepts valid image blob', () => {
    const ok = validateClipboardImageBlob({ size: 2048, type: 'image/png' });
