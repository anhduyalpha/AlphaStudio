...
describe('buildConversionGroups', () => {
  it('same-format files → mode batch, one group', () => {
    ...
    assert.ok(result.groups[0].outputs.some((o) => o.format === 'jpg'));
  ...
  it('retains unavailable outputs with reason for the board unavailable panel', () => {
    ...
    const png = unavailable.find((o) => o.format === 'png');
    ...
    assert.ok(group.outputs.some((o) => o.format === 'txt' && o.available));
    // Panel filter used by ConverterView
    const panel = (group.outputs || []).filter((o) => !o.available);
    assert.ok(panel.length > 0);
    assert.ok(panel.every((o) => Boolean(o.reason)));
  });
  ...
  it('compatibleOutputsForMembers intersects availability and keeps fail-closed reasons', () => {
    ...
    const avif = outs.find((o) => o.format === 'avif');
  ...
  it('unsupported / no detect → unsupported array', () => {
    ...
    const ids = result.unsupported.map((f) => f.id).sort();
...
describe('buildResultRows', () => {
  it('maps completed jobs with downloadUrl only when completed', () => {
    ...
    const completed = rows.find((r) => r.id === 'j1');
    assert.ok(completed);
