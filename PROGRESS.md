...
| 8 | C1 | Tokens and base styles | in-progress | unit-8-c1-tokens-base | — | — | resumed after both gate-2 blockers cleared: (1) the `run-checks.mjs` phase heuristic resolved itself when unit 5 landed `src/next/` (verified — token-purity now scans the new client only); (2) SPEC §4.1 `--text-3` amended to #798396 dark / #676d7b light (user-approved values) — the amendment was NOT in the user's commit 94df4af, which only deleted the stale token test, so it is made in this unit; evidence/unit-8.md |
