...
| 5 | B2 | protocol/store.ts | in-progress | unit-5-b2-protocol-store | — | 1,2 | spec review has now reported (background workflow, findings re-verified against source): 4 confirmed correctness defects — jobRank ranks completed below failed/cancelled, hydrateInFlight can strand, deletes leave no tombstone, epoch/seq dead ternary. Fixing all four then re-running gates; evidence/unit-5.md |
